    ...
    <nav class="navbar bg-light px-2">
        <div class="container-fluid">
            <div class="navbar-brand">
                <img src="{{ asset('img/logo/logo.png') }}" alt="Logo" width="120" height="40"
        ...
        nameInputs.forEach((nameInput, index) => {
            ...
            nameInput.addEventListener('drop', function(e) {
                ...
                items.forEach(item => {
                    if (item.textContent === text) {
                        item.classList.add(
                        'disabled'); // Menambahkan class 'disabled' pada drag item
                    }
                });
            });
            ...
            nameInput.addEventListener('input', function() {
                if (this.value !== "") {
                    ...
                    enableButton[index].style.display =
                    'none'; // Menyembunyikan tombol "Enable" jika input memiliki nilai

                    // Mengaktifkan item yang sesuai saat input diisi
                    items.forEach(item => {
                        if (item.textContent === this.value) {
                            item.classList.remove(
                            'disabled'); // Menghapus class 'disabled' pada drag item
                        }
                    });
                }
            });
