        ...
        <div class="row mx-2">
            <form action="{{ url('/multiple_choice/save_answer/' . $quiz->quiz_id) }}" method="POST" id="save_mc">
                ...
                @foreach ($multiple_choice as $item)
                    <input type="hidden" name="{{ 'user_id-' . $item->question_id }}" value="{{ auth()->user()->user_id }}">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mt-3 bg-secondary rounded" style="height: 30vh">
                                    <img src="{{ asset('storage/images/quiz/'.$item->question_image) }}" style="width: 100%; height: 100%; object-fit: contain" alt="">
                                </div>
                                @endif
                            </div>
                            <div class="card-footer">
                                @foreach ($options as $option)
                                    ...
                                    <input type="hidden" name="{{ 'key_answer-'.$item->question_id }}" value="{{ $option->key_answer }}">
                                    <input type="hidden" name="{{ 'question_poin-'.$item->question_id }}" value="{{ $item->question_poin }}">

                                    <p class="mb-1">
                                        <input type="radio" name="{{ 'answer-'.$item->question_id }}" id="{{ 'answer-'.$option->opsi1.$item->question_id }}" value="{{ $option->opsi1 }}">
                                        <label for="{{ 'answer-'.$option->opsi1.$item->question_id }}">{{ $option->opsi1 }}</label>
                                    </p>
                                    <p class="mb-1">
                                        <input type="radio" name="{{ 'answer-'.$item->question_id }}" id="{{ 'answer-'.$option->opsi2.$item->question_id }}" value="{{ $option->opsi2 }}">
                                        <label for="{{ 'answer-'.$option->opsi2.$item->question_id }}">{{ $option->opsi2 }}</label>
                                    </p>
                                    <p class="mb-1">
                                        <input type="radio" name="{{ 'answer-'.$item->question_id }}" id="{{ 'answer-'.$option->opsi3.$item->question_id }}" value="{{ $option->opsi3 }}">
                                        <label for="{{ 'answer-'.$option->opsi3.$item->question_id }}">{{ $option->opsi3 }}</label>
                                    </p>
                                    <p class="mb-1">
                                        <input type="radio" name="{{ 'answer-'.$item->question_id }}" id="{{ 'answer-'.$option->opsi4.$item->question_id }}" value="{{ $option->opsi4 }}">
                                        <label for="{{ 'answer-'.$option->opsi4.$item->question_id }}">{{ $option->opsi4 }}</label>
                                    </p>
                                    <p class="mb-1">
                                        <input type="radio" name="{{ 'answer-'.$item->question_id }}" id="{{ 'answer-'.$option->opsi5.$item->question_id }}" value="{{ $option->opsi5 }}">
                                        <label for="{{ 'answer-'.$option->opsi5.$item->question_id }}">{{ $option->opsi5 }}</label>
                                    </p>
                                    @endif
                                @endforeach
                            </div>
